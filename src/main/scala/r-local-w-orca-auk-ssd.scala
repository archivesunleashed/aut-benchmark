...
val validPages = RecordLoader
  .loadArchives("/scratchSSD/nruest/5467/warcs", sc)
  .keepValidPages()

validPages
  .map(r => ExtractDomainRDD(r.getUrl))
  .countItems()
  .saveAsTextFile("/scratchSSD/nruest/5467/all-domains/output")

validPages
  .map(r => (r.getCrawlDate, r.getDomain, r.getUrl, RemoveHTMLRDD(RemoveHTTPHeaderRDD(r.getContentString))))
  .saveAsTextFile("/scratchSSD/nruest/5467/all-text/output")
...
WriteGraph.asGraphml(links, "/scratchSSD/nruest/5467/gephi/output.graphml")
